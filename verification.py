...
def sigrecog():
    ...
    try:
        ...
        savingpath ="D:/signatureverificationphotos/org";
        savingpath1 ="D:/signatureverificationphotos/tst";
        savingpath = savingpath+"/"
        stable_kpt_from_refs = surf_feat(savingpath)
        #rt = abc(savingpath)
        threshold = 0.11
        savingpath1 = savingpath1+"/"
        genuine_matches = classify(savingpath1+"/", stable_kpt_from_refs, threshold)
        eer = 0.03
        gen_conf = np.where(genuine_matches < eer)

        #print(genuine_matches.shape[0] - gen_conf[0].shape[0])
        removeallfiles(savingpath1)
        removeallfiles(savingpath)
        if genuine_matches.shape[0] - gen_conf[0].shape[0]>0 :
            return "matched"
        else:
            return "Not Matched"
